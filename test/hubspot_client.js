...
import { HubspotClient, cleanAndCompareWithSchema, MISSING_NAME_PLACEHOLDER } from '../src/hubspot_client';
...
describe('HubspotClient', () => {
    describe('constructor()', () => {
        it('can be initialized', () => {
            ...
            expect(hubspotClient.config).to.be.eql(BASE_CONFIG);
        });
        it('fails if config is missing or incomplete', () => {
            ...
            config = {};
            expect(throwingFunction).to.throw(); // Api key is missing
            config.apiKey = BASE_CONFIG.apiKey;
            expect(throwingFunction).to.throw(); // Invoice object id is missing
            config.invoiceObjectId = BASE_CONFIG.invoiceObjectId;
            expect(throwingFunction).to.throw(); // Invoice to contact association is missing
        });
    });
    ...
    describe('searchContactByEmail()', () => {
        ...
        it('outputs contact info for existing contact', async () => {
            ...
            expect(data.id).to.be.deep.equal(contactId);
            expect(data.properties).to.be.deep.equal(networkReply.results[0].properties);
        });
        ...
        it('throws if email is empty', async () => {
            try {
                ...
                expect(error.message).to.be.equal('Arg "email" is required in HubspotClient.searchContactByEmail');
            }
        });
    });
    ...
    describe('searchContactByApifyUserId()', () => {
        ...
        it('throws if userId is not passed is empty', async () => {
            try {
                ...
                expect(error.message).to.be.equal('Arg "apifyUserId" is required in HubspotClient.searchContactByApifyUserId');
            }
        });
    });
    ...
    describe('lookupContactId()', () => {
        ...
        it('outputs contact ID info if contact exists with apify ID set', async () => {
            ...
            expect(data).to.be.deep.equal(contactId);
        });
    ...
    describe('getContact()', () => {
        ...
        it('outputs contact info for existing contact', async () => {
            ...
            expect(data.id).to.be.deep.equal(contactId);
            expect(data.properties).to.be.deep.equal(networkReply.properties);
        });
    ...
    describe('createContact()', () => {
        ...
        it('creates new contact in hubspot', async () => {
            ...
            expect(contactId).to.be.equal(networkReply.id);
        });
    });
    ...
    describe('updateContact()', () => {
        ...
        it('correctly handles not found error', async () => {
            ...
            expect(err.message).to.be.equal('Hubspot record not found');
        });
    });
    ...
    describe('searchInvoiceByNumber()', () => {
        ...
        it('throws if invoiceNumber is empty', async () => {
            try {
                ...
                expect(error.message).to.be.equal('Arg "invoiceNumber" is required in HubspotClient.searchInvoiceByNumber');
            }
        });
    });
    ...
    describe('searchInvoiceByApifyInvoiceId()', () => {
        ...
        it('throws if userId is not passed is empty', async () => {
            try {
                ...
                expect(error.message).to.be.equal('Arg "apifyInvoiceId" is required in HubspotClient.searchInvoiceByApifyInvoiceId');
            }
        });
    });
    ...
    describe('createInvoice()', () => {
        ...
        it('creates new invoice in hubspot', async () => {
            ...
            const transformedData = hubspotClient._transformInvoice(invoiceData, true);
            ...
            expect(invoiceId).to.be.equal(expectedInvoiceId);
        });
        it('throws if contact does not exist', async () => {
            ...
            try {
                ...
                expect(error.message).to.be.equal(`Hubspot with contact ID ${expectedContactId} does not exist`);
            }
        });
    });
    ...
    describe('createLead()', () => {
        ...
        it('creates new lead in hubspot', async () => {
            ...
            expect(contactId).to.be.equal(expectedLeadId);
        });
    });

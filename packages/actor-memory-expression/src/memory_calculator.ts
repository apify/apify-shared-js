...
import { LruCache, type LruCacheOptions } from '@apify/datastructures';
...
/**
 * Class for calculating dynamic run memory based on user-defined expressions.
 * Supports caching of compiled expressions for improved performance.
 *
 * Note: if cacheOptions are not provided, caching is disabled.
 */
export class RunMemoryCalculator {
    private readonly cache?: LruCache<EvalFunction>;
    private readonly compile: (expression: string) => EvalFunction;

    constructor(cacheOptions?: LruCacheOptions) {
        this.cache = cacheOptions ? new LruCache(cacheOptions) : undefined;
        this.compile = this.initializeCompileFunction();
    }

    private initializeCompileFunction() {
        /**
    ...
    */
    private customGetFunc(obj: any, path: string, defaultVal?: number) {
    ...
    */
    private roundToClosestPowerOf2(num: number): number {
     ...
     */
    private processTemplateVariables(defaultMemoryMbytes: string): string {
    ...
    */
    private getCompiledExpression(expression: string): EvalFunction {
        if (!this.cache) {
            return this.compile(expression);
        }

        let compiledExpression = this.cache.get(expression);

        if (!compiledExpression) {
            compiledExpression = this.compile(expression);
            this.cache.add(expression, compiledExpression!);
        }
    ...
    */
    calculateRunDynamicMemory(
        ...
        const preprocessedExpression = this.processTemplateVariables(defaultMemoryMbytes);

        const preparedContext = {
            ...context,
            get: this.customGetFunc,
        };

        const compiledExpression = this.getCompiledExpression(preprocessedExpression);
        ...
        return this.roundToClosestPowerOf2(finalResult);
    }
}
